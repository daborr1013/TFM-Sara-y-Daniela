<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="utf-8">
    <title><?= $tituloPagina ?></title>
    <meta name="author" content="Sarita y Danielita"> 
    <meta name="keywords" content="web, personal">
    <meta name="description" content="Litterally">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/css/bootstrap.min.css"
      rel="stylesheet"
      integrity="sha384-sRIl4kxILFvY47J16cr9ZwB07vP4J8+LH7qKQnuqkuIAvNWLzeN8tE5YBujZqJLB"
      crossorigin="anonymous">

    <link rel="stylesheet" href="<?= PATH_INDEX . '../css/css_plantilla.css' ?>" />
</head>

<body>

    <div id="contenedor">
        <?php
            include(PATH_VISTAS . 'common/header.php');
            include(PATH_VISTAS . 'common/nav.php');
        ?>

        <main>
            <?= $contenidoPrincipal ?>
        </main>

        <?php
            include(PATH_VISTAS . 'common/aside.php');
            include(PATH_VISTAS . 'common/footer.php');
        ?>
    </div> 

    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/js/bootstrap.bundle.min.js"
        integrity="sha384-FKyoEForCGlyvwx9Hj09JcYn3nv7wiPVlz7YYwJrWVcXK/BmnVDxM+D2scQbITxI"
        crossorigin="anonymous"></script>

    <script src="<?= PATH_INDEX . 'js/rol.js' ?>"></script>
</body>
</html>